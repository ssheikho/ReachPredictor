...
ComputeConic2D[inPtsCart2D_]:= (
	...
	(*where ccConicSol= (a b c d e f)^T is the conic C represented as a 6-vector*)
...
ComputeParametricEllipseFromConic2D[inConicSolution_]:=(
	...
	{cpeEigenVals, cpeEigenVecs} = Eigensystem[cpeA33];
	{
		...
		axAlpha2->ArcCot[(inConicSolution[[1]]-inConicSolution[[3]])/inConicSolution[[2]]]/2.0
	}
)
...
ComputePlane[inMat_]:=(
	(* SVD solves for AX=0, where A contains all the points that lie on the plane, and X is
...
ComputeTrackPlane[inTracks_, inTrackIndex_]:= ComputePlane[
(* only take the data from the specified marker *)
inTracks[[inTrackIndex,2]]];


ComputeTrackPlaneData[inTracks_, inTrackIndex_]:= (
	ctpdPlane =  ComputeTrackPlane[inTracks, inTrackIndex];
	(*ctpdPlane[[2,2]] returns the plane unit normal vector*)
	(*gives the matrix that rotates the plane unit normal vector to the direction
	 of the z-axis in any dimension.*)
	...
	minOffset = Min[offsetPts2D[[3]]];
	offsetDiff = maxOffset - minOffset;
	...
	ctpdPts2D = {offsetPts2D[[1]], offsetPts2D[[2]]};
	{
		...
		CTPMeanOffset->offsetDiff,
		CTPPts2D->ctpdPts2D,
		CTPOffsetDiff->offsetDiff
	}
)
